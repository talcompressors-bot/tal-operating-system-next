...
- Use the existing `http://localhost:3000` server for validation whenever possible.
- Start or restart the dev server only when Liad explicitly requests it or reports that the server is no longer running.
