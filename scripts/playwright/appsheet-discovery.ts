...
const storageState = process.env.APPSHEET_STORAGE_STATE;
const headless = String(process.env.APPSHEET_DISCOVERY_HEADLESS || 'true').toLowerCase() !== 'false';
...
async function detectBlocker(page: Page): Promise<DiscoveryReport['blockers'][number] | null> {
  ...
  if (url.includes('accounts.google') || bodyText.includes('sign in') || bodyText.includes('login')) {
    return {
      type: 'LOGIN_REQUIRED',
      message: 'AppSheet requires login before discovery can continue.',
      instruction: 'Log in manually and provide APPSHEET_STORAGE_STATE, or run with APPSHEET_DISCOVERY_HEADLESS=false and complete login locally.'
    };
  }
  ...
  if (bodyText.includes('access denied') || bodyText.includes('permission') || bodyText.includes('not authorized')) {
    return {
      ...
      message: 'The current session does not have permission to view the AppSheet app.',
      instruction: 'Use an account with read access to the AppSheet app.'
    };
  }
...
async function collectView(page: Page, id: string): Promise<DiscoveryView> {
  ...
  const screenshotName = `appsheet-view-${id}.png`;
...
async function main() {
  ...
  if (!appUrl) {
    ...
    fs.writeFileSync(path.join(outputDir, 'playwright-discovery-report.example.json'), JSON.stringify(report, null, 2));
