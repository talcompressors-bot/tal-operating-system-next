...
2. Build hygiene for the existing missing Playwright dependency/type gap, if explicitly selected.
3. Optional Wave 2 import approval package, only if explicitly approved.

Project completion should not be overstated: infrastructure readiness is high for the staging/Prisma/Wave 1 path; read-only UI coverage is progressing through shells, central work screens, and preview intelligence; production automation readiness remains not started/gated.
