...
Project Brain Knowledge Base Consolidation is complete as a documentation-only refactor. Case-specific and one-time evidence files for SCR 40PM inventory/service-kit work, ServiceReport 5806 commercial evidence, and the ServiceReport-to-Maven link registry were moved to `project-brain/archive/research/`. Durable knowledge was consolidated into reusable Knowledge Bases: `MANUFACTURER_PARTS_REGISTRY.md`, `MANUFACTURER_SERVICE_KITS.md`, `SERVICE_COMMERCIAL_RULES.md`, `SERVICE_MAVEN_MAPPING.md`, `DOCUMENT_ENGINE.md`, and `SKU_MATCHING_RULES.md`. `KNOWLEDGE_BASE_CONSOLIDATION_REPORT.md` records the migration report. `PROJECT_OPERATING_PROTOCOL.md` now requires case-specific documents to remain temporary research only and requires permanent knowledge to expand reusable Knowledge Bases instead of creating permanent `*_EVIDENCE_PACKET`, `*_DISCOVERY`, `*_SPEC`, per-model, or per-service-report files. Wave 3 Maven Knowledge Layer remains active and Wave 2 remains frozen except bug fixes. The complete ServiceReport `5806` internal chain remains validated: AI Draft Preview -> trusted pricing evidence display -> protected internal BusinessDocument creation -> BusinessDocument review/approval -> protected Maven AutomationCommand creation -> AutomationCommand queue/detail review -> Maven dry-run -> protected line resolution -> final Maven dry-run validation -> internal HTML preview -> internal temporary PDF download -> runtime manufacturer SKU matching. Project mode remains `CAPABILITY_BUILDING`. Governance status is `FROZEN`. Current blocker: none for the Project Brain Knowledge Base Consolidation. Real Maven execution is still an explicit `APPROVAL_REQUIRED` gate and is not approved. Maven/Invoice4U calls, command execution, saved PDF persistence, customer-facing delivery, DB writes outside explicitly approved protected flows, email/customer-facing action, inventory action, production workflow work, schema changes, imports, runtime changes, and source-system actions remain gated and require separate explicit approval.
