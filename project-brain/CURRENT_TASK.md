...
## Latest Executive Runtime POC

Tal Executive Runtime POC Phase 1 is implemented as read-only support tooling under `tools/tal-executive-runtime/`. It does not call OpenAI yet, does not execute Codex, does not write to the database, does not mutate Git history, and does not touch Maven/Invoice4U, email, inventory, production, `app/**`, `lib/**`, or `prisma/**`.

Completed commits:

- `e22dfcc Add tal executive runtime poc skeleton`
- `ceb4f26 Implement executive runtime observe validation`
- `4a72b55 Fix executive runtime company objectives extraction`
- `783765d Fix executive runtime observe warnings`

Validation:

- Working Python command: `C:\Users\משתמש\AppData\Local\Programs\Python\Python312\python.exe`.
- Observe command: `C:\Users\משתמש\AppData\Local\Programs\Python\Python312\python.exe tools/tal-executive-runtime/executive_runtime.py --observe --dry-run`.
- Observe status after `783765d`: `GREEN`.
- Evidence collected: Git, Project Brain, PROJECT_SYNC, runtime map, and company objectives.
- Missing evidence: none.
- Stale/conflict warnings: none.
- Generated Python cache files are ignored and removed from the working tree.

Current blocker:

- none for the Tal Executive Runtime Observe phase.

Exact next task:

- No runtime implementation is approved by this POC. The next ERP capability must be selected through the Chief Architect evidence loop before Builder/Codex starts. Historical Project Brain candidates include Asset Workspace / Asset Timeline and further ServiceReport -> BusinessDocument approval/readiness hardening; the Chief Architect may challenge either if business evidence shows a higher-value path.
...
Tal Executive Runtime Observe validation is complete. The read-only POC now gathers Git state, Project Brain state, PROJECT_SYNC state, runtime-route evidence, and company objectives before any future AI decision. The latest validation status is `GREEN`; no missing evidence or stale/conflict warnings remain.

Current blocker: none for the Observe phase. Runtime implementation remains unapproved until a Chief Architect decision selects the next company capability and minimal task slice. Remaining gates: schema/enum expansion before true Tax Invoice / Receipt, Purchase Order, Delivery Note, or Debit Note support; FinancialEvidence/payment evidence before receipt collection; source BusinessDocument before credit workflow; Inventory/Procurement runtime before purchase or delivery workflows; explicit real Maven/Invoice4U approval; separate customer-facing/email approval; separate inventory approval; and separate schema/DB/storage approval for any persisted queue, evidence, attachment, correction type, or follow-up state.
...
## Operations Command Center Sprint 5

Implemented as `SAFE_LOCAL_IMPLEMENTATION` in commit `be1fada Add operations command center`.

Capability:

- `/operations` is now the TAL Operations Command Center, an action-driven morning work queue rather than only a dashboard.
- The screen remains an orchestration view over existing BusinessCase, Commercial, Financial, Approval, Automation, and read-only Inventory-impact runtime.
- Queue rows are projections of existing BusinessCases; they are not separate persisted tasks and do not introduce a generic task manager.

Runtime:

- Replaced one-bucket classification with queue projections for My Work Today, Immediate Attention, High Priority, Waiting For Technician, Waiting For Customer, Waiting For Approval, Waiting For Financial Action, Waiting For Parts, and Ready To Close.
- Rows still expose customer, asset, BusinessCase, current stage, current blocker, next recommended action, priority, assigned owner, last activity, commercial status, and financial status.
- Added an internal Open Action link back to the BusinessCase context; no workflow action is executed from the queue.
- Updated `/operations` page copy, summary cards, navigation text, route map entry, and table styling.

Validation:

- Project TypeScript still fails only on pre-existing unrelated `app/ai-drafts/ai-draft-adapter.ts` pricing-evidence typing issues; no Operations Command Center, BusinessCase, Commercial, Financial, Approval, Automation, or touched-file TypeScript errors appeared.
- `git diff --check` passed with CRLF warnings only.
- Unsandboxed read-only route validation passed:
  - `/operations` HTTP `200`.
  - `/business-cases/service-report/1e25bbb1` HTTP `200`.
  ...
  - `/business-documents/NEXT-AI-DRAFT-5806/preview` HTTP `200`.
  - `/business-documents/NEXT-AI-DRAFT-5806/pdf` HTTP `200`, `application/pdf`, `59730` bytes.
- Content validation confirmed Operations Command Center, Morning work queue, My Work Today, Immediate Attention, High Priority, Waiting For Technician, Waiting For Customer, Waiting For Approval, Waiting For Financial Action, Waiting For Parts, Ready To Close, all required row columns, Open Action, BusinessCase link `/business-cases/service-report/1e25bbb1`, Financial status `Receipt draft ready for approval`, Commercial status `Ready For External Adapter`, queue projection boundary text, and no-execute recommendation text.
- Review and preview totals remained `1885.00 ILS`, `320.45 ILS`, and `2205.45 ILS`; checked review/preview HTML did not expose manufacturer SKU `901165`.
- Boundary stress check found no new Prisma writes, schema changes, package installs, external adapter calls, OCR, bank API, receipt issuing, external accounting, email/customer action, inventory mutation, cloud/source-system action, or production behavior.

Exit review:

- TAL can realistically use this screen every morning to identify action queues, blockers, owners, waiting states, financial/commercial status, and the next recommended action.
- It is not yet a full operational execution system because persistent owner assignment, due dates/SLA, user-specific work ownership, "handled today" state, and writable close/resolve actions are not implemented. Those require a future approved schema/DB-write capability.
- Recommended Sprint 6, if staying in safe read-only capability mode: Customer Workspace / Customer Timeline, because the Command Center tells the office what needs attention and customer context is the next highest-value place to resolve that work.
- Alternative Sprint 6, if Liad wants the Command Center itself to become writable: Internal queue-state persistence for assignment/due dates/handled state, requiring explicit `SCHEMA_OR_DATA_CHANGE` approval.

Project completion:

- Moves to `75%` by adding one current workflow/orchestration point for the action-driven Operations Command Center.
- No schema, DB, external adapter, OCR, bank API, receipt issuing, external accounting, inventory mutation, customer action, cloud action, production readiness, writable task management, or real Maven/Invoice4U execution point is claimed.

## Customer 360 Workspace Sprint 6

Implemented as `SAFE_LOCAL_IMPLEMENTATION` in commit `32bb35e Add Customer 360 workspace`.

Capability:

- `/customers/[id]` is now a Customer 360 Workspace that answers "What is happening with this customer right now?"
- The workspace composes Party, Assets, BusinessCases, Service Operations, Commercial, Financial, Approval, Automation, Timeline, and Future Opportunities from existing runtime.
- It is a read-only projection, not a CRM engine, not a new customer engine, and not a duplicate BusinessCase or Operations Command Center.

Runtime:

- Extended the existing customer adapter with `getCustomer360ById()`, which reuses `getCustomerById()` and `getBusinessCaseList()`.
- Customer 360 shows Customer Summary, Assets, Open Business Cases, Open Commercial Documents, Financial Status, Current Blockers, Recommended Next Action, Recent Timeline, and Future Opportunities.
- Financial/document amounts use the existing BusinessDocument ViewModel totals so Customer 360 stays aligned with review, preview, and PDF totals.
- Updated `APPLICATION_ROUTE_MAP.md` for `/customers/[id]`.

Validation:

- Project TypeScript still fails only on pre-existing unrelated `app/ai-drafts/ai-draft-adapter.ts` pricing-evidence typing issues; no Customer 360, BusinessCase, Commercial, Financial, Approval, Automation, or touched-file TypeScript errors appeared.
- `git diff --check` passed with CRLF warnings only.
- Unsandboxed read-only route validation passed:
  - `/customers/18953` HTTP `200`.
  - `/customers` HTTP `200`.
  - `/operations` HTTP `200`.
  - `/business-cases/service-report/1e25bbb1` HTTP `200`.
  - `/business-documents/NEXT-AI-DRAFT-5806` HTTP `200`.
  - `/business-documents/NEXT-AI-DRAFT-5806/preview` HTTP `200`.
  - `/business-documents/NEXT-AI-DRAFT-5806/pdf` HTTP `200`, `application/pdf`, `59730` bytes.
- Content validation confirmed Customer 360, Customer Summary, Assets, Open Business Cases, Open Commercial Documents, Financial Status, Current Blockers, Recommended Next Action, Recent Timeline, Future Opportunities, BusinessCase, Commercial, Financial, Approval, Automation, read-only projection boundary, no-CRM boundary, BusinessCase link `/business-cases/service-report/1e25bbb1`, BusinessDocument link `/business-documents/NEXT-AI-DRAFT-5806`, `Ready For External Adapter`, `Receipt draft ready for approval`, and `2205.45 ILS`.
- Review and preview totals remained `1885.00 ILS`, `320.45 ILS`, and `2205.45 ILS`; checked review/preview HTML did not expose manufacturer SKU `901165`.
- Boundary stress check found no new Prisma writes, schema changes, package installs, external adapter calls, CRM workflow, OCR, bank API, receipt issuing, external accounting, email/customer action, inventory mutation, cloud/source-system action, or production behavior.

Exit review:

- TAL can now understand a customer relationship from one workspace: current customer state, affected equipment, open BusinessCases, commercial state, financial state, blockers, next action, recent timeline, and future opportunities are visible without opening multiple screens.
- The workspace cannot yet manage the entire relationship end-to-end because it has no persistent CRM notes, owner assignment, due dates/SLA, customer communication log, customer approval/send actions, or write-side follow-up workflow.
- Recommended Sprint 7, if staying safe/read-only: Asset Workspace / Asset Timeline, because Customer 360 now shows affected equipment and the next highest value is compressor-level history across service, commercial, financial, recommendations, and blockers.
- Alternative Sprint 7, if Liad wants execution rather than context: writable Command Center or Customer follow-up state, requiring explicit `SCHEMA_OR_DATA_CHANGE` approval.

Project completion:

- Moves to `76%` by adding one customer-relationship workspace capability point.
- No schema, DB, external adapter, OCR, bank API, receipt issuing, external accounting, inventory mutation, customer action, cloud action, production readiness, CRM workflow, or real Maven/Invoice4U execution point is claimed.
...
## ERP Sprint 7 - End-to-End Service Flow MVP

Implemented as `SAFE_LOCAL_IMPLEMENTATION` in commit `e8cb765 Add end-to-end service flow projection`.

Capability delivered:

- Added an End-to-End Service Flow projection to `BusinessCaseViewModel`.
- `/business-cases/service-report/[id]` now exposes Completed Service Report -> BusinessCase -> Business Intelligence Analysis -> Business Suggestions -> Commercial Runtime -> BusinessDocument Draft -> Operations Command Center -> Customer 360 -> Approval Ready.
- Business Intelligence Analysis answers what happened, what evidence exists, what evidence is missing, recommended business action, recommended BusinessDocument type, approvals required, financial implications, and inventory implications.
- Business Suggestions remain internal only and link back to existing AI draft, BusinessDocument, ServiceReport, Operations, and Customer 360 context where available.
- The projection reuses existing BusinessCase, ServiceReport, BusinessDocument, Commercial lifecycle, Financial Intake, Approval, Automation, Operations Command Center, Customer 360, and read-only Inventory-impact runtime.
- Updated `APPLICATION_ROUTE_MAP.md` for the BusinessCase route.

Boundary:

- No schema change, Prisma migration, DB write, package install, OCR, external AI, Maven/Invoice4U execution, email/customer-facing action, inventory mutation, cloud/source-system action, production behavior, or new business engine was introduced.
- The service flow is a read-only projection and explanation layer. It does not create BusinessDocuments, issue receipts, execute adapters, send messages, or mutate inventory.
- ServiceReport `5806` / BusinessDocument `NEXT-AI-DRAFT-5806` remains validation/regression evidence only, not architecture.

Validation:

- Project TypeScript still fails only on pre-existing unrelated `app/ai-drafts/ai-draft-adapter.ts` pricing-evidence typing issues; no BusinessCase, Commercial, Financial, Approval, Automation, Operations, Customer 360, route-map, or touched-file TypeScript errors appeared.
- `git diff --check` passed with CRLF warnings only.
- Unsandboxed read-only route validation passed:
  - `/service-reports/1e25bbb1` HTTP `200`.
  - `/business-cases/service-report/1e25bbb1` HTTP `200`.
...
- Content validation confirmed End-to-End Service Flow, Business Intelligence Analysis, Business Suggestions, Completed Service Report, BusinessCase, BusinessDocument Draft, Operations Command Center, Customer 360, Approval Ready, all required intelligence questions, linked BusinessDocument draft explanation, selected-type explanation, Customer 360 context, Operations Command Center context, and `NEXT-AI-DRAFT-5806`.
- Customer 360 still showed Customer 360, Open Business Cases, BusinessDocument, and `2205.45 ILS`.
- Operations Command Center still showed Operations Command Center, My Work Today, and Business case for Service Report `5806`.
- Review and preview totals remained `1885.00 ILS`, `320.45 ILS`, and `2205.45 ILS`; checked review/preview HTML did not expose manufacturer SKU `901165`.

Exit review:

- TAL can now follow the validated internal business process from Service Report `5806` to BusinessCase, Business Intelligence Analysis, Business Suggestions, linked BusinessDocument draft, Operations Command Center, Customer 360, and Approval Ready without leaving the ERP or switching modules manually.
- The process is not yet fully general for every completed Service Report because generalized internal draft creation from Business Suggestions is not implemented; existing draft creation remains governed by the current AI Draft approval/write gate and validated fixture path.
- Smallest missing capability: generalized Business Suggestion -> BusinessDocument Draft creation gate for non-5806 ServiceReports, with explicit DB-write approval and no external execution.
- Recommended Sprint 8 if Liad wants end-to-end creation: Generalized Business Suggestion to BusinessDocument Draft Gate.
- Recommended Sprint 8 if staying read-only/context-first: Asset Workspace / Asset Timeline.

Project completion:

- Moves to `77%` by adding one integration/orchestration capability point to Wave 3 Commercial Runtime and Document Engine.
- No schema, DB, external adapter, OCR, bank API, receipt issuing, external accounting, inventory mutation, customer action, cloud action, production readiness, CRM workflow, or real Maven/Invoice4U execution point is claimed.

## ERP Sprint 8 - Generalized BusinessDocument Draft Gateway

Implemented as `SCHEMA_OR_DATA_CHANGE` in commit `36299a8 Add business document draft gateway`.

Capability delivered:

- Added `lib/business-document-draft-gateway.ts` as the reusable BusinessDocument Draft Gateway.
- Gateway requires explicit approval phrase `CREATE BUSINESS DOCUMENT DRAFT`, selected schema-supported document type, complete Business Intelligence, pricing review, confidence review, missing-evidence review, and pricing override when any line is review-required.
- Gateway idempotency is ServiceReport + DocumentType through deterministic `appsheetBusinessDocumentId` `NEXT-BD-DRAFT-{ServiceReportAppSheetId}-{DocumentType}` plus a defensive `serviceReportId + documentTypeSelected` lookup.
- Gateway writes only internal `BusinessDocument`, `BusinessDocumentItem`, and `BusinessDocumentLog` rows.
- Existing AI Draft Preview creation path now delegates to the gateway instead of owning the write mechanics.
- `/business-cases/service-report/[id]` now exposes the protected BusinessDocument Draft Gateway form for current-schema document types only: `QUOTE`, `INVOICE`, `RECEIPT`, `SERVICE_DOCUMENT`, `CREDIT_NOTE`, and `OTHER`.
- Updated `APPLICATION_ROUTE_MAP.md` for the BusinessCase route.
...
- This sprint performed one approved internal DB-write validation to create a non-5806 internal draft and one second submit to prove idempotency.
- The gateway does not decide the document type. The user selects the document type from current schema support.

Validation:

- Approved non-5806 write validation used ServiceReport `5802`, route/source id `9981b978`, Prisma id `8d889aeb-61ab-4b66-9316-516af5cd683c`, customer route id `18779`.
- First gateway submit created `NEXT-BD-DRAFT-9981b978-QUOTE`; count for ServiceReport + `QUOTE` changed from `0` to `1`.
- Second gateway submit redirected to the same draft with `draftStatus=existing`; count stayed `1`.
- Created draft validation:
  - `documentTypeSelected=QUOTE`.
  - `status=WAITING_USER_APPROVAL`.
  - `approvalStatus=PENDING`.
  - one `BusinessDocumentItem` was created.
  - line is `needsPriceApproval=true`.
  - audit log action `BUSINESS_DOCUMENT_DRAFT_GATEWAY_CREATED` exists.
  - zero linked `AutomationCommand` rows.
  - zero linked `InventoryTransaction` rows.
- Route validation passed:
  - `/business-cases/service-report/9981b978` HTTP `200`.
  - `/operations` HTTP `200`.
  - `/customers/18779` HTTP `200`.
  - `/business-documents/NEXT-BD-DRAFT-9981b978-QUOTE` HTTP `200`.
  - `/business-documents/NEXT-BD-DRAFT-9981b978-QUOTE/preview` HTTP `200`.
  - `/business-documents/NEXT-BD-DRAFT-9981b978-QUOTE/pdf` HTTP `200`, `application/pdf`.
- Regression validation passed:
  ...
  - Manufacturer SKU `901165` was not exposed in checked 5806 review/preview HTML.
- Project TypeScript still fails only on pre-existing unrelated `app/ai-drafts/ai-draft-adapter.ts` pricing-evidence typing issues; no gateway, BusinessCase action/page, AI preview action, BusinessDocument route, or touched-file TypeScript errors appeared.
- `git diff --check` passed with CRLF warnings only.

Exit review:

- The gateway can create future BusinessDocument drafts for any document type already represented by the current `BusinessDocumentType` enum and safely render them through existing BusinessDocument review/preview/PDF runtime.
- It cannot yet support true Tax Invoice / Receipt, Purchase Order, Delivery Note, or Debit Note as first-class document types simply by configuration, because those enum values do not exist in the current schema and some require domain-specific ownership beyond Commercial.
- Smallest remaining gap: schema/document-type expansion plus domain-specific line/source configuration for unsupported future types when explicitly approved.

Project completion:

- Moves to `78%` by adding one Wave 3 Commercial Runtime and Document Engine capability point for generalized internal draft creation.
...
1. Asset Workspace / Asset Timeline: expose compressor-level service, commercial, financial, recommendation, blocker, and customer history using existing runtime.
2. BusinessDocument type expansion design, if Liad wants true Tax Invoice / Receipt, Purchase Order, Delivery Note, or Debit Note as first-class draft types; requires explicit schema/enum approval before implementation.
3. Internal queue-state persistence for the Operations Command Center or Customer 360, if explicitly selected: add assignment, due dates/SLA, user-specific work, and handled/resolve state only after schema/DB-write approval.
4. Inventory and Procurement boundary/readiness: define how parts, procurement, supplier, stock, reservations, and future delivery-note work interact with Service Operations, Commercial documents, Financial status, and BusinessCase without inventory mutation.
5. FinancialEvidence persistence and attachment storage, if explicitly selected: design and implement persisted evidence, duplicate registry, storage/privacy/retention, and approved issuing workflow only after schema/DB/storage approval.
...
- Added `lib/business-document-production-draft.ts` as a knowledge-first production draft recommendation layer.
- `/business-cases/service-report/[id]` now shows Production Draft Recommendation, confidence, Knowledge First evidence, generated lines, missing evidence, and estimated manual-work reduction before user approval.
- BusinessCase draft creation now feeds evidence-backed reviewable lines into the existing BusinessDocument Draft Gateway instead of creating one generic `Needs approval` line.
- The generator exhausts current customer, compressor model, compressor family, manufacturer, maintenance interval, BusinessCase type, service-kit, spare-part, approved BusinessDocumentItem, and approved correction evidence before marking fields unknown.
- The existing Business Intent Policy Layer remains above the gateway and the existing BusinessDocument Draft Gateway remains the only writer.
- Gateway source mapping now distinguishes same-customer history, same-equipment history, and general similar-service approved history instead of treating all historical BusinessDocumentItems as same-customer evidence.

Boundary:

- No Prisma schema change, enum extension, migration, package install, Maven/Invoice4U execution, email/customer-facing action, inventory mutation, OCR, bank API, receipt issuing, external adapter call, cloud/source-system action, or production behavior occurred.
- This sprint performed approved internal DB-write validation only through the existing protected gateway to create internal BusinessDocument drafts.
- No external actions were executed. Drafts remain internal, `WAITING_USER_APPROVAL`, and `PENDING`.

Validation:

- Focused TypeScript/project TypeScript still fails only on pre-existing unrelated `app/ai-drafts/ai-draft-adapter.ts` pricing-evidence typing issues; no touched-file TypeScript errors appeared.
- `git diff --check` passed with CRLF warnings only.
- Real non-5806 ServiceReports validated:
  - `5864` / `acd1133d` created `NEXT-BD-DRAFT-acd1133d-QUOTE`.
  - `5863` / `891a1dd7` created `NEXT-BD-DRAFT-891a1dd7-QUOTE`.
  - `5861` / `e645e483` created `NEXT-BD-DRAFT-e645e483-QUOTE`.
- Repeated submit for `e645e483` returned `draftStatus=existing`; ServiceReport + `QUOTE` count stayed `1`.
- Corrected source mapping validation for `NEXT-BD-DRAFT-e645e483-QUOTE`:
  - generated two reviewable lines: `Technician Visit / Travel` and `Labor + Service`.
  - proposed prices came from approved historical BusinessDocumentItem evidence.
  - `source=SIMILAR_SERVICE_HISTORY`, not same-customer history.
  - zero linked `AutomationCommand` rows.
  - zero linked `InventoryTransaction` rows.
  - zero linked `EmailLog` rows.
- Route validation passed:
  - `/business-cases/service-report/e645e483` HTTP `200`.
  - `/operations` HTTP `200`.
  - `/customers/18779` HTTP `200`.
...
- TAL can now generate reviewable internal BusinessDocument drafts from real ServiceReports faster than manually starting from a blank document.
- Drafts are intentionally not perfect. Confidence, missing evidence, and review-required status are visible before approval.
- Existing approved historical line corrections successfully eliminated manual first-pass pricing lookup for travel and labor/service lines in the validated drafts.
- Smallest remaining gap: close the production draft review/learning loop so approved user corrections from generated drafts become durable append-only future evidence through existing review, line-resolution, and approval runtime.

Project completion:

- Moves to `80%` by adding one Wave 3 Commercial Runtime and Document Engine capability point for production draft generation from real ServiceReports.
- No external adapter, OCR, bank API, receipt issuing, external accounting, inventory mutation, customer action, cloud action, production readiness, CRM workflow, or real Maven/Invoice4U execution point is claimed.

Next recommended task, pending explicit selection/approval:

Production Draft Review and Learning Loop: reuse existing BusinessDocument review, line resolution, approval, and historical BusinessDocumentItem evidence so generated drafts can be corrected quickly and approved corrections reduce future editing effort. Asset Workspace / Asset Timeline remains the next best safe context capability if Liad prioritizes asset history over draft learning.

## ERP Sprint 9 - Business Intent Policy Layer

Implemented as `SAFE_LOCAL_IMPLEMENTATION` in commit `5e8a504 Add business intent policy layer`.

Capability delivered:

- Added `lib/business-intent-policy.ts` as the policy layer above the existing BusinessDocument Draft Gateway.
- Initial supported intents are `PROPOSE_WORK`, `BILL_CUSTOMER`, `COLLECT_PAYMENT`, `CREDIT_CUSTOMER`, `PURCHASE_PARTS`, `DELIVER_GOODS`, `WARRANTY_REVIEW`, and `SCHEDULE_FOLLOW_UP`.
- Each intent defines owner domain, required evidence, allowed document types, blocked conditions, approval requirements, and whether it may create a BusinessDocument draft now.
- `/business-cases/service-report/[id]` now requires Business Intent before draft creation and routes user-selected intent plus selected document type through Domain Policy before calling the existing gateway.
- Allowed now: `PROPOSE_WORK -> QUOTE` and `BILL_CUSTOMER -> INVOICE` as internal drafts only. `BILL_CUSTOMER` requires completed billable work confirmation and explicit internal override when policy requires it.
- Blocked now with policy reasons: `COLLECT_PAYMENT` requires FinancialEvidence/payment evidence; `CREDIT_CUSTOMER` requires a source BusinessDocument; `PURCHASE_PARTS` requires future Inventory/Procurement; `DELIVER_GOODS` requires future Delivery Note/schema support; `WARRANTY_REVIEW` and `SCHEDULE_FOLLOW_UP` do not create document drafts by default.

Boundary:

- No Prisma schema change, enum extension, migration, package install, Maven/Invoice4U execution, email/customer-facing action, inventory mutation, OCR, bank API, receipt issuing, external adapter call, cloud/source-system action, or production behavior occurred.
- The only DB write performed in Sprint 9 was the approved internal gateway validation for `BILL_CUSTOMER -> INVOICE`, creating or reusing `NEXT-BD-DRAFT-9981b978-INVOICE`. Existing `PROPOSE_WORK -> QUOTE` idempotency reused `NEXT-BD-DRAFT-9981b978-QUOTE`.
- The policy layer does not write drafts. The existing BusinessDocument Draft Gateway remains the only writer and keeps ServiceReport + DocumentType idempotency.

Validation:

- Approved non-5806 validation used ServiceReport `5802`, route/source id `9981b978`, Prisma id `8d889aeb-61ab-4b66-9316-516af5cd683c`, customer route id `18779`.
- `PROPOSE_WORK + QUOTE` returned the existing idempotent draft `NEXT-BD-DRAFT-9981b978-QUOTE`.
- `BILL_CUSTOMER + INVOICE` created or reused internal draft `NEXT-BD-DRAFT-9981b978-INVOICE`.
- Blocked-intent validation passed: `COLLECT_PAYMENT + RECEIPT`, `CREDIT_CUSTOMER + CREDIT_NOTE`, `PURCHASE_PARTS + OTHER`, and `DELIVER_GOODS + OTHER` all redirected with `draftGatewayStatus=intent-blocked`.
- Route validation passed for `/business-cases/service-report/9981b978`, `/operations`, `/customers/18779`, `/business-documents/NEXT-BD-DRAFT-9981b978-QUOTE`, `/business-documents/NEXT-BD-DRAFT-9981b978-QUOTE/preview`, `/business-documents/NEXT-BD-DRAFT-9981b978-QUOTE/pdf`, `/business-documents/NEXT-BD-DRAFT-9981b978-INVOICE`, `/business-documents/NEXT-BD-DRAFT-9981b978-INVOICE/preview`, and `/business-documents/NEXT-BD-DRAFT-9981b978-INVOICE/pdf`.
- Regression validation passed for `/business-cases/service-report/1e25bbb1`, `/business-documents/NEXT-AI-DRAFT-5806`, `/business-documents/NEXT-AI-DRAFT-5806/preview`, and `/business-documents/NEXT-AI-DRAFT-5806/pdf`.
- Existing 5806 totals remained `1885.00 ILS`, `320.45 ILS`, and `2205.45 ILS`; manufacturer SKU `901165` was not exposed in checked 5806 review/preview HTML.
- Read-only Prisma side-effect check found exactly two target drafts for ServiceReport `5802`: `NEXT-BD-DRAFT-9981b978-QUOTE` and `NEXT-BD-DRAFT-9981b978-INVOICE`; each has one item, zero AutomationCommands, and zero InventoryTransactions.
- Project TypeScript still fails only on pre-existing unrelated `app/ai-drafts/ai-draft-adapter.ts` pricing-evidence typing issues; no touched-file TypeScript errors appeared.
- `git diff --check` passed with CRLF warnings only.

Exit review:

- Business Intent is now the correct decision boundary above document type selection. The ERP no longer asks only "which document type?" when creating a draft; it first asks "what business outcome is intended?" and lets policy restrict document choices.
- The implementation is intentionally conservative: it supports current-schema `QUOTE` and `INVOICE` drafts, while blocking payment, credit, procurement, delivery, warranty, and follow-up intents until their owning domain evidence and runtime exist.
- Future document types can be enabled by extending policy and document type configuration only when schema/enums and owning-domain validations exist. True Tax Invoice / Receipt, Purchase Order, Delivery Note, and Debit Note still require explicit schema/enum and domain-specific approval before implementation.

Project completion:

- Moves to `79%` by adding one Wave 3 Commercial Runtime and Document Engine capability point for intent-driven draft policy.
- No external adapter, OCR, bank API, receipt issuing, external accounting, inventory mutation, customer action, cloud action, production readiness, CRM workflow, or real Maven/Invoice4U execution point is claimed.
