...
| `/business-documents/[id]` | BusinessDocument Review and Approval Page | Implemented internal draft review page with protected Maven draft command gate; GET shows source ServiceReport link, BusinessDocumentItems, pricing evidence, approval-required warnings, command status, and explicit Internal Draft / Not sent / No Maven action / No inventory deduction states; POST Server Action can create one internal `CREATE_MAVEN_DRAFT` AutomationCommand only after exact approval phrase when BusinessDocument status is Approved or Ready To Send | Supabase PostgreSQL via Prisma `BusinessDocument` with linked items/logs/customer/service-report/AI-draft/Maven/AutomationCommand context when present; POST writes `AutomationCommand` only and performs no Maven/Invoice4U call, email/customer-facing action, inventory action, schema change, or source-system action | 0+ addressable business document rows after approved internal draft creation | `BusinessDocuments` detail/review and Maven draft command approval gate |
