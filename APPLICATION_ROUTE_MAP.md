...
| `/business-documents/[id]` | Business Document detail | Implemented, read-only shell/detail; created AI preview draft items display preserved pricing evidence and price-approval state | Supabase PostgreSQL via Prisma `BusinessDocument` with linked items/logs/customer/service-report/AI-draft/Maven context when present | 0+ addressable business document rows after approved internal draft creation | `BusinessDocuments` detail view |
