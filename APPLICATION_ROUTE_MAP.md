...
| `/business-documents/[id]` | BusinessDocument Review and Approval Page | Implemented, read-only internal draft review page; shows source ServiceReport link, all BusinessDocumentItems, preserved pricing evidence, approval-required warnings, and explicit Internal Draft / Not sent / No Maven action / No inventory deduction states | Supabase PostgreSQL via Prisma `BusinessDocument` with linked items/logs/customer/service-report/AI-draft/Maven context when present | 0+ addressable business document rows after approved internal draft creation | `BusinessDocuments` detail/review view |
