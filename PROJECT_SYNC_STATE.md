...
## Sync Fingerprint
(shared closeout marker used to compare Git, Project Brain, PROJECT_SYNC, Drive mirror, and ChatGPT Project Sources)

Sync Token
(the unique token generated by the mirror script for the latest end-of-task sync)
Written by `scripts/sync-project-mirror.ps1` as `SYNC-YYYYMMDD-HHMM-<shortCommit>` in `SYNC_FINGERPRINT.txt`.

Git Commit
(the Git `HEAD` that defines committed repo truth)
Resolved by `git rev-parse HEAD` during closeout and written to `SYNC_FINGERPRINT.txt`.

Project Brain Version
(the `HEAD` after required Project Brain updates)
Must equal Git Commit when Project Brain updates were required; for sync-only tasks with no Project Brain update required, it still records the current closeout `HEAD`.

PROJECT_SYNC Version
(the `HEAD` after required `PROJECT_SYNC_*` updates)
Must equal Git Commit after sync-file updates are committed and pushed.

Drive Mirror Version
(the `HEAD` copied by the Drive mirror script)
Must equal Git Commit after `scripts/sync-project-mirror.ps1` completes and writes `SYNC_FINGERPRINT.txt`.

Project Sources Version
(the ChatGPT Project Sources upload/refresh state)
`MANUAL_CONFIRM_REQUIRED` unless Liad confirms the files were uploaded/refreshed in ChatGPT Project Sources.

Last Verified
(when the shared fingerprint was written and checked)
Written by `scripts/sync-project-mirror.ps1` in ISO timestamp form inside `SYNC_FINGERPRINT.txt`.

Sync Status
(GREEN / YELLOW / RED result for the latest shared fingerprint)
`GREEN` only when Git Commit, Project Brain Version, PROJECT_SYNC Version, Drive Mirror Version, and confirmed Project Sources Version match the same `HEAD`. `YELLOW` when Git/Project Brain/PROJECT_SYNC/Drive Mirror match but Project Sources manual confirmation is missing or Google Drive cloud status cannot be read. `RED` when Git, Project Brain, PROJECT_SYNC, or Drive Mirror disagree.
