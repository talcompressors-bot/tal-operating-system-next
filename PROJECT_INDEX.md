...
| Current task | Evidence Graph Validation Gate is implemented in `4c49755`: `lib/business-knowledge-engine.ts` now validates graph contracts and exposes a recommendation gate that blocks consumers unless required graph coverage is present and missing evidence sources/data-quality gaps are listed. `tools/evidence-graph-validation.ts` validates controlled fixtures, cross-provider behavior, authority levels, negative cases, `buildEvidenceGraph()`, gate allow/block behavior, and a read-only runtime asset graph. No AI Draft or ServiceReport recommendation consumer was migrated; no schema, DB write/import, Maven/Invoice4U, email/customer action, inventory mutation, source-system, package/parser install, persisted graph storage, architecture expansion, dashboard, SKU selection, recommendation logic, or production action occurred | `project-brain/CURRENT_TASK.md`, `project-brain/TASK_BOARD.md`, `project-brain/TAL_INTELLIGENCE_CORE.md` |
| Next approved task | Use `validateKnowledgeGraph()` and `evaluateEvidenceGraphRecommendationGate()` before any recommendation consumer relies on graph output. Recommended safe candidates: consume `getAssetEvidenceGraph()` in Asset Intelligence/service analysis without recommendation logic, or expand read-only provider retrieval depth for inventory/Maven/product graph evidence. Real Maven execution for `NEXT-MAVEN-CMD-NEXT-AI-DRAFT-5806` requires separate explicit Liad approval and the readiness checklist in `project-brain/CURRENT_TASK.md`. DB writes outside approved protected Server Actions remain gated and require explicit human approval | `project-brain/CURRENT_TASK.md`, `project-brain/TASK_BOARD.md`, `DATA_COVERAGE_AUDIT.md` |
...
- Current Task: Evidence Graph Validation Gate is implemented before recommendation engines may rely on the graph.
- Last Completed Task: Evidence Graph validation and recommendation gate in `4c49755`, after the Evidence Correlation Engine in `38a6162`.
- Next Task: Use `validateKnowledgeGraph()` and `evaluateEvidenceGraphRecommendationGate()` before any graph consumer, either by consuming `getAssetEvidenceGraph()` in Asset Intelligence/service analysis without recommendation logic or by expanding read-only provider retrieval depth for inventory/Maven/product graph evidence.
...
| Current task | Evidence Graph Validation Gate is implemented before recommendation engines may rely on the graph. The graph remains read-only, in-memory, evidence-only, and recommendation-free; the gate validates graph contracts and blocks consumers when required node, relationship, or source coverage is missing. Real Maven execution, receipt issuing, OCR, bank API, saved PDF persistence, customer delivery/export, inventory action, schema changes, DB imports, parser implementation, parser package installs, LibreOffice install/conversion, persisted graph storage, and new npm package installs without package-specific approval are not approved | `project-brain/CURRENT_TASK.md`, `project-brain/TASK_BOARD.md`, `PROJECT_SYNC_STATE.md`, `PROJECT_SYNC_DELTA.md`, `PROJECT_SYNC_AGENTS.md`, `PROJECT_SYNC_TASKS.md`, `PROJECT_SYNC_AUTHORITY.md`, `PROJECT_SYNC_OPERATING_GUIDE.md` |
| Next approved task | Domain-driven next task selection is active. Recommended next safe/context capability is using `validateKnowledgeGraph()` and `evaluateEvidenceGraphRecommendationGate()` in any graph consumer, consuming the evidence graph in Asset Intelligence/service analysis without recommendation logic, or expanding read-only provider retrieval depth for inventory/Maven/product graph evidence. Later valid candidates include migrating ServiceReport-to-BusinessDocument production draft generation to retrieve validated correlated evidence through the Business Knowledge Engine, addressing the `PartsUsed=0` evidence gap through approved import/data work, correction persistence type expansion if Liad wants deeper write-side learning, BusinessDocument type expansion design if unsupported future document types are selected, Inventory and Procurement boundary/readiness, FinancialEvidence persistence/attachment storage if explicitly selected, Automation and Integration adapter readiness, or BusinessCase runtime generalization if a future capability proves it necessary. Real Maven execution for `NEXT-MAVEN-CMD-NEXT-AI-DRAFT-5806` requires separate explicit Liad approval and the readiness checklist in `project-brain/CURRENT_TASK.md`. DB writes outside approved protected Server Actions remain gated and require explicit human approval | `project-brain/CURRENT_TASK.md`, `project-brain/TASK_BOARD.md`, `DATA_COVERAGE_AUDIT.md` |
