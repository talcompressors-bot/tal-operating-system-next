...
| Current task | Known TypeScript pricing-evidence typing issue in `app/ai-drafts/ai-draft-adapter.ts` is fixed with a narrow non-null `flatMap` construction change. Business logic is unchanged, full TypeScript validation passes, and Maven/Invoice4U, email/customer action, inventory mutation, schema change, package install, DB write, and external action remain untouched | `project-brain/CURRENT_TASK.md`, `project-brain/TASK_BOARD.md`, `PROJECT_SYNC_STATE.md`, `PROJECT_SYNC_DELTA.md`, `PROJECT_SYNC_TASKS.md` |
