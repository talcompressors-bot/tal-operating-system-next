...
| Current task | Business Knowledge Engine Evidence Correlation Engine is implemented in `38a6162`: `lib/business-knowledge-engine.ts` now provides an in-memory unified evidence graph over engine-returned evidence with reusable node, edge, trace, confidence, and data-quality-gap contracts. Graph coverage includes customer, equipment, model, series, service reports, business documents/items, parts used, official model parts catalog, products catalog, inventory, Maven history, pricing history, supplier knowledge, and source-document fallbacks. No schema, DB write/import, Maven/Invoice4U, email/customer action, inventory mutation, source-system, package/parser install, persisted graph storage, architecture expansion, dashboard, SKU selection, recommendation logic, or production action occurred | `project-brain/CURRENT_TASK.md`, `project-brain/TASK_BOARD.md`, `project-brain/TAL_INTELLIGENCE_CORE.md` |
| Next approved task | Use the Evidence Correlation Engine as the required graph boundary before migrating additional recommendation consumers. Recommended safe candidates: expose/consume `getAssetEvidenceGraph()` in Asset Intelligence/service analysis without recommendation logic, or expand read-only provider retrieval depth for inventory/Maven/product graph evidence. Real Maven execution for `NEXT-MAVEN-CMD-NEXT-AI-DRAFT-5806` requires separate explicit Liad approval and the readiness checklist in `project-brain/CURRENT_TASK.md`. DB writes outside approved protected Server Actions remain gated and require explicit human approval | `project-brain/CURRENT_TASK.md`, `project-brain/TASK_BOARD.md`, `DATA_COVERAGE_AUDIT.md` |
...
| Last Implementation Commit | `38a6162 Add evidence correlation graph` | Git history; `project-brain/CURRENT_TASK.md` |
...
- Current Task: Business Knowledge Engine Evidence Correlation Engine is implemented as the unified graph layer over engine-returned evidence.
- Last Completed Task: Evidence Correlation Engine in `38a6162`, after the Business Knowledge Engine foundation and first Asset Intelligence consumer migration in `aac294f`.
- Next Task: Use the evidence graph as the required boundary before more recommendation migration, either by consuming `getAssetEvidenceGraph()` in Asset Intelligence/service analysis without recommendation logic or by expanding read-only provider retrieval depth for inventory/Maven/product graph evidence.
