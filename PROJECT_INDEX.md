...
| Last Implementation Commit | `f420701 Add BusinessCase runtime` | Git history; `project-brain/CURRENT_TASK.md` |
...
- Current Task: BusinessCase Runtime Sprint 1 complete; the ERP now has a read-only operational case spine derived from existing ServiceReport context and existing domain runtimes.
- Last Completed Task: BusinessCase Runtime Sprint 1
- Next Task: Select the next ERP capability. Recommended safe candidates are BusinessCase runtime generalization, Commercial document lifecycle hardening, Financial Intake and Settlement readiness, Inventory and Procurement boundary planning, or Automation and Integration adapter readiness. Maven customer/document/item matching and Maven API evidence are valid only when selected as External Adapter readiness tasks, not as the default architecture path.
...
- Wave 3 Commercial Runtime and Document Engine: 6% / 15% STARTED INTERNAL
...
- Completion formula: 15 + 15 + 10 + 10 + 15 + 6 + 0 + 0 + 0 + 0 = 71.
