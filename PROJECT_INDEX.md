...
| Current milestone | TDOS is frozen in Maintenance Mode and project focus has returned to ERP capability implementation. Asset Intelligence is now a reasoning engine on the Equipment detail screen and can produce traceable senior-service conclusions from existing evidence. BusinessCase Runtime Sprint 1, Commercial Lifecycle Hardening Sprint 2, Financial Capability Sprint 3, Operations Center Sprint 4, Operations Command Center Sprint 5, Customer 360 Workspace Sprint 6, End-to-End Service Flow MVP Sprint 7, Generalized BusinessDocument Draft Gateway Sprint 8, Business Intent Policy Layer Sprint 9, Production Draft Generation Sprint 10, Production Draft Review and Learning Loop Sprint 11, and Autonomous Business Draft Generation Sprint 12 are implemented. Domain-driven Commercial, Financial, customer, operational workspace, service-flow integration, draft-gateway, intent-policy, production draft generation, approved-correction learning, autonomous Hebrew draft generation, and asset runtime intelligence work is started. Maven is classified as an External Adapter under Automation and Integration, not as the architectural center. Wave 2 Service Workflow Layer is complete, frozen except bug fixes, and architecture-audited | `project-brain/CURRENT_TASK.md`, `project-brain/TASK_BOARD.md` |
| Current task | P1 Business Knowledge Coverage Plan is documented in `DATA_COVERAGE_AUDIT.md` before any recommendation engine migration. The plan lists every missing P1 source with current coverage %, runtime availability, parsed/indexed/searchable/correlated/ranked status, Evidence Graph use, and blockers. Production recommendation migration is blocked until official catalog, customer/equipment/service, pricing, PartsUsed, Product Catalog, Maven history, inventory/supplier, rejected-alternative, and data-quality evidence is available through the Business Knowledge Engine or explicitly blocked by the recommendation gate. No AI Draft or ServiceReport recommendation consumer was migrated; no schema, DB write/import, Maven/Invoice4U, email/customer action, inventory mutation, source-system, package/parser install, persisted graph storage, architecture expansion, dashboard, SKU selection, recommendation logic, or production action occurred | `DATA_COVERAGE_AUDIT.md`, `project-brain/CURRENT_TASK.md`, `project-brain/TASK_BOARD.md` |
| Next approved task | Implement the smallest read-only P1 provider-depth improvement in the Business Knowledge Engine, starting with populated/local evidence that needs no schema/import/source-system action: BusinessDocumentItem pricing history and approved correction evidence, or official manufacturer catalog fixture completeness/ranking. Do not migrate draft generation yet. Real Maven execution for `NEXT-MAVEN-CMD-NEXT-AI-DRAFT-5806` requires separate explicit Liad approval and the readiness checklist in `project-brain/CURRENT_TASK.md`. DB writes outside approved protected Server Actions remain gated and require explicit human approval | `DATA_COVERAGE_AUDIT.md`, `project-brain/CURRENT_TASK.md`, `project-brain/TASK_BOARD.md` |
...
| Last Implementation Commit | `4c49755 Add evidence graph validation gate` | Git history; `project-brain/CURRENT_TASK.md` |
...
- Current Task: P1 Business Knowledge Coverage Plan is documented before recommendation migration.
- Last Completed Task: Evidence Graph validation and recommendation gate in `4c49755`, followed by P1 Business Knowledge Coverage Plan in `DATA_COVERAGE_AUDIT.md`.
- Next Task: Implement the smallest read-only P1 provider-depth improvement in the Business Knowledge Engine for populated/local evidence, preferably BusinessDocumentItem pricing history and approved corrections or official manufacturer catalog fixture completeness/ranking. Do not migrate draft generation yet.
...
- Completion formula: 15 + 15 + 10 + 10 + 15 + 15 + 1 + 0 + 0 + 0 = 81.
- Critical Path: P1 Business Knowledge Coverage provider depth -> Evidence Graph gate coverage for every production recommendation source -> later recommendation consumer migration through validated correlated evidence -> ServiceReport-to-BusinessDocument recommendation improvement through unified evidence -> PartsUsed evidence gap/import only if separately approved -> correction persistence/type expansion if deeper draft learning requires it -> BusinessDocument type expansion design if unsupported future document types are selected -> Inventory and Procurement boundaries -> FinancialEvidence persistence/attachments when explicitly approved -> Operations Command Center / Customer 360 follow-up persistence if explicitly approved -> Automation and Integration adapter readiness -> External Adapter evidence packets, including Maven/Invoice4U when selected -> Offline Field Runtime -> Production Shadow -> Production Cutover and AppSheet Retirement
...
| Current task | P1 Business Knowledge Coverage Plan is documented before recommendation migration. The plan identifies missing P1 coverage for official catalog completeness, PartsUsed, Product Catalog, pricing/corrections, Maven history, inventory, suppliers, Google Sheets/AppSheet source rows, technical Excel catalogs, and PDFs. Real Maven execution, receipt issuing, OCR, bank API, saved PDF persistence, customer delivery/export, inventory action, schema changes, DB imports, parser implementation, parser package installs, LibreOffice install/conversion, persisted graph storage, and new npm package installs without package-specific approval are not approved | `DATA_COVERAGE_AUDIT.md`, `project-brain/CURRENT_TASK.md`, `project-brain/TASK_BOARD.md`, `PROJECT_SYNC_STATE.md`, `PROJECT_SYNC_DELTA.md`, `PROJECT_SYNC_AGENTS.md`, `PROJECT_SYNC_TASKS.md`, `PROJECT_SYNC_AUTHORITY.md`, `PROJECT_SYNC_OPERATING_GUIDE.md` |
| Next approved task | Implement the smallest read-only P1 Business Knowledge Engine provider-depth improvement using populated/local evidence that needs no schema/import/source-system action. Recommended candidates are BusinessDocumentItem pricing history plus BusinessDocumentLog approved corrections, or official manufacturer catalog fixture completeness/ranking. Draft generation must not be migrated yet. Later valid candidates include addressing the `PartsUsed=0` evidence gap through approved import/data work, Product Catalog provider depth after data exists, Maven history after approved import/read-only source access, Inventory and Procurement boundary/readiness, FinancialEvidence persistence/attachment storage if explicitly selected, Automation and Integration adapter readiness, or BusinessCase runtime generalization if a future capability proves it necessary. Real Maven execution for `NEXT-MAVEN-CMD-NEXT-AI-DRAFT-5806` requires separate explicit Liad approval and the readiness checklist in `project-brain/CURRENT_TASK.md`. DB writes outside approved protected Server Actions remain gated and require explicit human approval | `DATA_COVERAGE_AUDIT.md`, `project-brain/CURRENT_TASK.md`, `project-brain/TASK_BOARD.md` |
