...
| Current milestone | TDOS is frozen in Maintenance Mode and project focus has returned to ERP capability implementation. BusinessCase Runtime Sprint 1 and Commercial Lifecycle Hardening Sprint 2 are implemented without schema changes. Domain-driven Wave 3 Commercial Runtime and Document Engine work is started internal. Universal Business Document Engine foundation, Domain Boundary Refactoring, Financial Intake Engine design, internal BusinessDocument/payment review, Hebrew customer-facing BusinessDocument preview/PDF, manufacturer SKU Excel source planning, ServiceReport 5806 SKU matching runtime MVP, Manufacturer Parts Registry Import + Service Kit Intelligence MVP, Manufacturer Registry Alias and Conflict Validation, Legacy Excel Parser Strategy, Maven sample reference asset, internal PDF export planning, MVP, smoke test, governance lock, and visual polish fixes, Maven payload builder extraction, Maven terminology correction, Maven source inventory, and Maven API placeholders are complete. Maven is classified as an External Adapter under Automation and Integration, not as the architectural center. Wave 2 Service Workflow Layer is complete, frozen except bug fixes, and architecture-audited | `project-brain/CURRENT_TASK.md`, `project-brain/TASK_BOARD.md` |
| Current task | Commercial Lifecycle Hardening Sprint 2 completed as `SAFE_LOCAL_IMPLEMENTATION` in commit `c8d8c6c Harden commercial document lifecycle`. The runtime now derives a complete internal Commercial lifecycle from existing BusinessDocument, Approval, Automation, and Financial-readiness signals: Draft, Internal Review, Needs Changes, Approved, Ready For External Adapter, Externally Processed, Financial Pending, Financial Completed, and Closed. It does not create schema, DB writes, external adapter execution, Maven/Invoice4U calls, email/customer actions, inventory mutation, cloud action, or production behavior | `project-brain/CURRENT_TASK.md`, `project-brain/TASK_BOARD.md`, `project-brain/DECISION_LOG.md`, `APPLICATION_ROUTE_MAP.md` |
| Next approved task | ERP capability implementation remains active. Recommended Sprint 3 is Financial Intake and Settlement readiness because Commercial can now hand off financial-pending/future-completed states without redesign. Maven customer/document/item matching, Maven API contract evidence, and Maven secret placement remain valid only as External Adapter readiness tasks when explicitly selected. Real Maven execution for `NEXT-MAVEN-CMD-NEXT-AI-DRAFT-5806` requires separate explicit Liad approval and the readiness checklist in `project-brain/CURRENT_TASK.md`. DB writes outside approved protected Server Actions remain gated and require explicit human approval | `project-brain/CURRENT_TASK.md`, `project-brain/TASK_BOARD.md`, `DATA_COVERAGE_AUDIT.md` |
...
| Last Implementation Commit | `c8d8c6c Harden commercial document lifecycle` | Git history; `project-brain/CURRENT_TASK.md` |
...
- Current Task: Commercial Lifecycle Hardening Sprint 2 complete; BusinessDocument now exposes a complete derived internal lifecycle while preserving existing approval, preview, PDF, automation-gate, and BusinessCase behavior.
- Last Completed Task: Commercial Lifecycle Hardening Sprint 2
- Next Task: Recommended Sprint 3 is Financial Intake and Settlement readiness. It should start from current Commercial handoff state and remain schema/write/external-action gated unless explicitly approved.
...
- Wave 3 Commercial Runtime and Document Engine: 7% / 15% STARTED INTERNAL
...
- Completion formula: 15 + 15 + 10 + 10 + 15 + 7 + 0 + 0 + 0 + 0 = 72.
- Critical Path: Financial Intake and Settlement readiness -> Inventory and Procurement boundaries -> Automation and Integration adapter readiness -> External Adapter evidence packets, including Maven/Invoice4U when selected -> Offline Field Runtime -> Production Shadow -> Production Cutover and AppSheet Retirement
