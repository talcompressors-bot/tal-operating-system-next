...
| Google Drive mirror status | Approved Drive mirror readback | Codex with Google Drive connector after commit/push; human owner defines target | Human owner, ChatGPT Project Sources maintainer, Codex | End of task after sync files changed or mandatory sync refresh; verify `01_Project_Sources` expected files, broad runtime-folder exclusion for `app` / `lib` / `prisma`, and Final Sync Status `GREEN` / `YELLOW` / `RED` | Drive mirror for current `PROJECT_SYNC_*` files and approved knowledge folders only | ChatGPT Project Sources may lag behind committed sync state or accidentally receive broad runtime/source folders |
